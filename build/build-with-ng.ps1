...
$webHostFolder = Join-Path $slnFolder "src/Htsc.Shared.Web.Host"
$webPublicFolder = Join-Path $slnFolder "src/Htsc.Shared.Web.Public"
...
docker rmi htscsharedwebhost -f
...
docker rmi htscsharedwebpublic -f
docker compose -f docker-compose.yml build

# Angular UI
Set-Location (Join-Path $outputFolder "ng")

docker rmi htscsharedwebangular -f
docker compose -f docker-compose.yml build
